...
public class V1_server {
	...
	public Response getTrackInJSON() throws Exception {
		...
		DBCollection dBCollection = QueryDriver.getmongoDbCollection(
				"zaneacademydatabase", "Channel");
	...
	/*
	 * 
	 * PreparedStatement query = null; Connection conn = null; String
	 * returnString = null; Response rb = null;
	 * 
	 * try{ conn = MysqlClient.MySqlCom2Conn().getConnection(); query =
	 * conn.prepareStatement("select * from wearable.clientinfo"); ResultSet rs
	 * = query.executeQuery();
	 * 
	 * ToJSON converter = new ToJSON(); JSONArray json = new JSONArray();
	 * 
	 * json = converter.toJSONArray(rs); query.close(); //close connection
	 * 
	 * returnString = json.toString(); rb = Response.ok(returnString).build();
	 * 
	...
	public Response createTrackInJSON(String incomingData) throws Exception {
        ...
        DBCollection dBCollection = QueryDriver.getmongoDbCollection("lwm2mserver", "registerclient");
            
        QueryDriver.insertJSON(incomingData,dBCollection);
		
		returnString = "2.01 Created";
